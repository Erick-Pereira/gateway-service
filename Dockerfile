# Imagem a partir do diretório gateway-service/ (docker-compose usa este ficheiro).
# Alinhado com Simcag.Gateway.Api/Dockerfile (contexto: raiz gateway-service).
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
    CMD sh -c 'p="${ASPNETCORE_HTTP_PORTS:-8080}"; p="${p%%,*}"; curl -fsS "http://localhost:${p}/health/live" >/dev/null || exit 1'
